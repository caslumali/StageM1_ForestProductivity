...
## 1.2 ROI ----
# ---------------------------------------------------------
SITES <- c("Paragominas", "Cotriguacu", "Guaviare",  "MDD") # If you want to loop # 'Cotriguacu', 'Guaviare', 'MDD', 'Paragominas'
...
for (i in 1:length(SITES)) {
  ...
  write.csv(evi_stats_all, file.path(OUT_FOLDER, str_glue("{Site}_EVI_stats_all.csv")), row.names = FALSE)
  write.csv(acc_deg_stats_all, file.path(OUT_FOLDER, str_glue("{Site}_Acc_deg_stats_all.csv")), row.names = FALSE)
  write.csv(tw_deg_stats_all, file.path(OUT_FOLDER, str_glue("{Site}_Tw_deg_stats_all.csv")), row.names = FALSE)
  ...
  stacked_bar_plot(evi_plot_data, file.path(OUT_FOLDER, str_glue("{Site}_EVI_stacked_bar.png")), "Surface de la tendance de l'EVI (km²)", evi_periods, evi_labels)
  ...
  stacked_bar_plot(acc_deg_plot_data, file.path(OUT_FOLDER, str_glue("{Site}_Acc_deg_stacked_bar.png")), "Surface de pixels présentant une certaine dégradation (accumulée)", acc_deg_periods, acc_deg_labels)
  ...
  stacked_bar_plot(tw_deg_plot_data, file.path(OUT_FOLDER, str_glue("{Site}_Tw_deg_stacked_bar.png")), "Surface de pixels présentant une certaine dégradation (par période)", tw_deg_periods, tw_deg_labels)
