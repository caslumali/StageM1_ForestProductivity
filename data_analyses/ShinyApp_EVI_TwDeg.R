...
ui <- fluidPage(
  ...
  div(class='main-title', titlePanel("EVI Trends Analyses")),
  div(class = 'flex-container', # Div to centralize the items
      selectInput("timeWindow", "Choose the period:", choices = names(TW)),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    deg_values <- sapply(list(deg_2000_2005, deg_2003_2008, deg_2006_2011, deg_2009_2014, deg_2012_2017, deg_2015_2020), function(rast) {
      extract(rast, xy)
    })
  ...
  output$map <- renderLeaflet({
    leaflet() %>%
      ...
      addRasterImage(evi_class_2000_2005, colors = color_pal_class, opacity = 0.7, project = FALSE, group = "EVI Class 2000-2005") %>%
      addRasterImage(evi_class_2003_2008, colors = color_pal_class, opacity = 0.7, project = FALSE, group = "EVI Class 2003-2008") %>%
      addRasterImage(evi_class_2006_2011, colors = color_pal_class, opacity = 0.7, project = FALSE, group = "EVI Class 2006-2011") %>%
      addRasterImage(evi_class_2009_2014, colors = color_pal_class, opacity = 0.7, project = FALSE, group = "EVI Class 2009-2014") %>%
      addRasterImage(evi_class_2012_2017, colors = color_pal_class, opacity = 0.7, project = FALSE, group = "EVI Class 2012-2017") %>%
      addRasterImage(evi_class_2015_2020, colors = color_pal_class, opacity = 0.7, project = FALSE, group = "EVI Class 2015-2020") %>%
      addRasterImage(deg_2000_2005, colors = color_pal_deg, opacity = 0.7, project = FALSE, group = "Degradation 2000-2005") %>%
      addRasterImage(deg_2003_2008, colors = color_pal_deg, opacity = 0.7, project = FALSE, group = "Degradation 2003-2008") %>%
      addRasterImage(deg_2006_2011, colors = color_pal_deg, opacity = 0.7, project = FALSE, group = "Degradation 2006-2011") %>%
      addRasterImage(deg_2009_2014, colors = color_pal_deg, opacity = 0.7, project = FALSE, group = "Degradation 2009-2014") %>%
      addRasterImage(deg_2012_2017, colors = color_pal_deg, opacity = 0.7, project = FALSE, group = "Degradation 2012-2017") %>%
      addRasterImage(deg_2015_2020, colors = color_pal_deg, opacity = 0.7, project = FALSE, group = "Degradation 2015-2020") %>%
      addLayersControl(
        overlayGroups = c("EVI",
                          "EVI Class 2000-2005",
                          "EVI Class 2003-2008",
                          ...
                          "EVI Class 2012-2017",
                          "EVI Class 2015-2020",
                          "Degradation 2000-2005",
                          "Degradation 2003-2008",
                          "Degradation 2006-2011",
                          "Degradation 2009-2014",
                          "Degradation 2012-2017",
                          "Degradation 2015-2020"),
  ...
  observe({
    leafletProxy("map") %>%
      ...
      hideGroup("Degradation 2000-2005") %>%
      hideGroup("Degradation 2003-2008") %>%
      hideGroup("Degradation 2006-2011") %>%
      hideGroup("Degradation 2009-2014") %>%
      hideGroup("Degradation 2012-2017") %>%
      hideGroup("Degradation 2015-2020")
  })
  ...
  output$regressionPlot <- renderPlotly({
    ...
    if (!all(is.na(values))) {
      ...
      plot_ly() %>%
        add_ribbons(data = data_for_plot, x = ~year, ymin = ~lower_ci, ymax = ~upper_ci, name = "Confidence Interval",
                    fillcolor = 'rgba(204,204,204,0.5)', line = list(color = 'transparent')) %>%
        add_lines(data = data_for_plot, x = ~year, y = ~predicted_EVI_value, name = "Trend Line",
                  line = list(color = 'blue')) %>%
        add_markers(data = data_for_plot, x = ~year, y = ~EVI_value, name = "Observed EVI",
                    mode = "markers", marker = list(color = 'darkorange', size = 10)) %>%
        layout(yaxis = list(title = 'EVI Value',
  ...
  output$degradationValues <- renderTable({
    ...
    periods <- names(TW)
    data.frame(Period = periods, Degradation = as.numeric(deg_values))
  })
}
